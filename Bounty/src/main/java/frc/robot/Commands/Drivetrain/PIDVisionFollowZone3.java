...
public class PIDVisionFollowZone3 extends Command {
    ...
    // speed PID
    private double dPgain = 1.0; /* percent throttle per degree of error */ //was .04
    ...
    private double targetta = 1.12;  // needs to be found
    ...
    protected void execute() {
        ...
        if (Math.abs(taError) >= 0.1) {
            Robot.Drivetrain.drive(ControlMode.PercentOutput, left, right);
        }

        else {
            Robot.Drivetrain.drive(ControlMode.PercentOutput, left/5, right/5);
        }
        lastTimeStamp = Timer.getFPGATimestamp();
        lastError = taError;
    }
