    ...
    public Red2ConeBalanceAuto(Swerve s_Swerve){
        ...
        Trajectory ReturnRedCone1 =
            TrajectoryGenerator.generateTrajectory(
                ...
                //End one meter forward
                new Pose2d(0 * i2m, 22 * i2m, new Rotation2d(Math.toRadians(180))),
                configRev);

        Trajectory BalancePath =
            TrajectoryGenerator.generateTrajectory(
                // Start at the origin facing the +X direction
                new Pose2d(0 * i2m, 22 * i2m, new Rotation2d(Math.toRadians(180))),
                // Pass through these two interior waypoints, making an 's' curve path
                List.of(new Translation2d(25 * i2m, 82 * i2m)),
                //End one meter forward
                new Pose2d(65 * i2m, 82 * i2m, new Rotation2d(Math.toRadians(180))),
                configRev);
        ...
        addCommands(
            ...
            new InstantCommand(() -> s_Swerve.resetOdometry(BalancePath.getInitialPose())),
            //new FloorIntakeCommandGroup().alongWith(swerveControllerCommand).withTimeout(5)
