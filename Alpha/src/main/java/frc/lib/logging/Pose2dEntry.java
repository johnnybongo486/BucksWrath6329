...
public class Pose2dEntry {
  ...
  public Pose2dEntry(String name) {
    this(name, new Pose2d());
  }

  public Pose2dEntry(String name, Pose2d value) {
    this(name, value, false);
  }

  public Pose2dEntry(String name, Pose2d value, boolean logged) {
    this.pose = value;
    this.logged = logged;
    ...
    topic = LogManager.getNTInstance().getDoubleArrayTopic(name);
  ...
  public void set(Pose2d value) {
    ...
    pub.set(converted);
    if (LogManager.isCompetition() && logged) log.append(converted);
  }
